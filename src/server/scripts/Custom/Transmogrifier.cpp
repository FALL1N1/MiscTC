    ...
    void OnLogin(Player* player)
    {
        ...
        if (result)
        {
            do
            {
                ...
                {
					TC_LOG_ERROR("server.worldserver", "Item entry (Entry: %u, itemGUID: %u, playerGUID: %u) does not exist, ignoring.", fakeEntry, GUID_LOPART(itemGUID), player->GetGUIDLow());
...
public:
    ...
    {
        sT->LoadConfig(false);
		TC_LOG_ERROR("server.worldserver", "Deleting non-existing transmogrification entries...");
